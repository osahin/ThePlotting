...
void plotting::properties::readConfig(TString & config_path, int trial) {
  ...
  while(getline(config_file,line))
      ...
      if('#' != line[0]) //lines starting with "#" reserved for the comments
	{
	  ...
	  if(!(items == 2 || items == 3) ) {cout << "please check the config file structure: <sample path> (<sub dir>) <sample type>" << endl; exit(0);}
	  ...
	  if(items == 3) (*sub_dir) = ((TObjString*)next())->String(); else sub_dir = 0;
	  ...
	  else {cout << " The type of the sample is not defined properly (sig, dat, bkg) "<< endl; exit(0);} 
	}
    }
  config_file.close();
}
